...
const localRecommendCount = ref(props.note.recommendCount);
const localRecommendedByMe = ref(false);
const localRemainingRecommendCount = ref<number | null>(null);
...
const remainingRecommendationCount = computed(
  () => localRemainingRecommendCount.value ?? myRecommendations.value?.remainingCount ?? 0,
);
...
const likeMutation = useMutation({
  ...
  onError: (_err, _vars, context) => {
    if (context) {
      localLiked.value = context.prevLiked;
      localLikeCount.value = context.prevCount;
    }
  },
});
...
const recommendMutation = useMutation({
  ...
  onMutate: () => {
    const prevCount = localRecommendCount.value;
    const prevRemaining = remainingRecommendationCount.value;
    localRecommendCount.value += 1;
    localRecommendedByMe.value = true;
    localRemainingRecommendCount.value = prevRemaining - 1;
    return { prevCount, prevRemaining };
  },
  onSuccess: () => {
    queryClient.invalidateQueries();
    const remaining = remainingRecommendationCount.value;
    if (remaining <= 0) {
  ...
  onError: (_err, _vars, context) => {
    if (context) {
      localRecommendCount.value = context.prevCount;
      localRemainingRecommendCount.value = context.prevRemaining;
      localRecommendedByMe.value = false;
    }
  },
});
  ...
  >
    ...
    <div class="flex items-center gap-6 border-t px-5 py-3" style="border-color: #25253840">
      ...
      >
        ...
        <span class="text-sm font-mono">{{ localRecommendCount }}</span>
